...
func main() {
	...
	sort.Slice(crossing, func(i int, j int) bool {
		return abs(crossing[i].x)+abs(crossing[i].y) <
			abs(crossing[j].x)+abs(crossing[j].y)
	})
	fmt.Println("La solution de la partie 1 est", abs(crossing[0].x)+abs(crossing[0].y))
}
